# Arco by Goat Story — 2-in-1 Grinder

Nitrided steel Italmill conical burrs, 32 mm inner (outer diameter disputed — see Disputed below). One grinder, two drive modes: crank it by hand, or twist it into the Arco Power Dock (bayonet mount) and a motor turns the same burrs at 360 rpm.
...
- The zero point is a **mechanical property of the collar and burrs**, so it does not move when you dock the grinder. What is untested is whether the same *gap* yields the same *particle distribution* under motor drive — see Pending below

### Working range (electric)

- **Ignore the manufacturer's method labels** (chart below). For 58 mm espresso at real pressure you will need far finer than the "espresso" band. Settings below 0/30 — nominally "ibrik" territory — are routinely correct
...
- Every setting recorded in this journal was reached on the **electric** dock. The journal's espresso range so far is **0/26–0/32**

### Logging convention — electric is the default

**Every brew in this journal is electric-mode unless the entry explicitly says hand-ground.** Past and future, no exceptions.

- **All brews logged to date: electric.** Confirmed by the user, not inferred.
...
### Mechanical facts, hand vs electric

| Factor | Hand | Electric dock |
|---|---|---|
| Burr speed | Very low (~60–120 rpm, inferred — see Pending) | **360 rpm**, motor-held |
...
## Manufacturer-stated (sourced, not independently measured on this unit)

From the official Goat Story product page.
...
### Manufacturer grind chart (per method)
...
**The chart is mode-blind.** It gives one number per method and does not say which drive mode it was derived on. Do not assume it applies to both drive modes on this unit.
...
### Hand burr speed
Not published by the manufacturer. Hand cranking is roughly 60–120 rpm — *inference, not a source*.

### The rpm → fines quality claim
...
### Setting equivalence between modes
...
**Cheaper cross-check on the Hario Switch:** open mode, fixed dose/grind/water/temperature, compare **drawdown time**. Slower drawdown = more fines. Filter costs less per data point than espresso and removes puck prep as a confound. Do this first if beans are short. See `equipment/hario-switch.md`.
